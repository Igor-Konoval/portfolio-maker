...
// class Repository { //for begin remote controll data
...
class ContentEditor {
    ...
    constructor() {
        this.classHTTP = new XMLHttpRequest();
        console.log('constructor');
        console.log(this.classHTTP);
    }
    ...
    sendAjaxRequest(url) {
        this.classHTTP.open('GET', url);
        this.classHTTP.onload = () => {
            if (this.classHTTP.status === 200) {
                const data = JSON.parse(this.classHTTP.responseText);
                ...
                console.error('Ошибка получения данных: ' + this.classHTTP.status);
            }
            this.classHTTP.onerror = function () {
...
// const contentEditor = new ContentEditor(new HttpClient);
